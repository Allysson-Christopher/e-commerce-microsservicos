...
| [0011](ADR-0011-audit-baseline-aws-cloudtrail-config.md)                     | Audit baseline AWS via CloudTrail management-only + AWS Config minimal com `required-tags` escopada                                    | accepted                                                                                   | 2026-05-02 | infra, security, devsecops, audit, aws, compliance, learning-trade-off |
| [0012](ADR-0012-escopar-permission-set-iam-identity-center.md)               | Escopar permission set IAM Identity Center via policy híbrida (tag-based + ARN-enumerated), com `AdministratorAccess` como break-glass | accepted                                                                                   | 2026-05-02 | security, infra, iam, devsecops, aws, learning-trade-off               |
...
| -------------- | ------------------------------------------------------------------------------------------------------------------- | ------------- | ------------------------- |
| 0013           | Ansible para configuração de EC2 (Amazon Linux 2023) + OpenTofu para Cloudflare e AWS (VPC, IAM, EC2, EBS, EIP, SG) | §5.5          | P0-C2, P0-D1              |
| 0014           | Cloudflare proxied + Traefik + Let's Encrypt + mTLS origin (Authenticated Origin Pulls)                             | §7.2          | P0-D2 a P0-D4             |
| 0015           | OIDC trust GitHub Actions ↔ AWS IAM para CD (substitui `DEPLOY_SSH_KEY` em GitHub Secrets)                          | §5.3          | P0-F3                     |
| 0016           | Linguagem do hello-service                                                                                          | §9, P0-E1     | **P0-E1 (DoD da tarefa)** |
| 0017           | Stack de observabilidade Grafana LGTM + OpenTelemetry                                                               | §6.1          | P0-G1                     |
| 0018           | DevSecOps stack remanescente: Trivy + gitleaks-CI + Checkov + SonarCloud + cosign/SBOM (CodeQL já em **ADR-0007**)  | §6.3 + nota   | P0-H2 a P0-H5, P0-A5      |
| 0019           | Renovate para gestão de dependências                                                                                | §6.3          | P0-H4                     |
...
>
> **Histórico de remapeamento (2026-05-02 — sessão 2):** **ADR-0011**
> nasceu como "Audit baseline AWS via CloudTrail + Config" (decisões
> técnicas concretas do P0-B5 com alternativas reais avaliadas),
> consumindo o número 0011 que estava reservado para Ansible/OpenTofu.
> Cascata: Ansible/OpenTofu vai para 0012, e candidatas 0012-0017 desta
> seção shiftam +1. Adicionada candidata 0019 (escopar permission set
> IAM Identity Center) que nasce no PR de P0-B6 — sucessor natural de
> P0-B5 (auditoria primeiro, restrição depois, com auditoria capturando
> qualquer ação fora-do-escopo durante a transição).
>
> **Histórico de remapeamento (2026-05-02 — sessão 3):** **ADR-0012**
> nasceu como "Escopar permission set IAM Identity Center" (P0-B6,
> sucessor de P0-B5), consumindo o número 0012 que estava reservado
> para Ansible/OpenTofu. P0-B6 saiu da fila de candidatas (já tem ADR);
> Ansible/OpenTofu vai para 0013, e demais candidatas 0013-0018 desta
> seção shiftam +1.
